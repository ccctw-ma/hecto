...
pub struct Row {
    string: String,
    highlighting: Vec<highlightling::Type>,
...
impl Row {
    pub fn render(&self, start: usize, end: usize) -> String {
        ...
        let mut current_highlighting = &highlightling::Type::None;
            ...
            .take(end - start)
        {
            if let Some(c) = grapheme.chars().next() {
                let highlighting_type = self
                    .highlighting
                    .get(index)
                    .unwrap_or(&highlightling::Type::None);
            ...
            while let Some(search_match) = self.find(word, index, SearchDirection::Forward) {
                if let Some(next_index) = search_match.checked_add(word[..].graphemes(true).count())
                {
                    ...
                    for i in index.saturating_add(search_match)..next_index {
                        self.highlighting[i] = highlightling::Type::Match;
                    }
                    index = next_index;
                } else {
                    break;
    ...
    fn highlight_primary_keywords(
        ...
        self.highlight_keywords(
            ...
            highlightling::Type::PrimaryKeywords,
        )
    }
    fn highlight_secondary_keywords(
        ...
        self.highlight_keywords(
            ...
            highlightling::Type::SecondaryKeywords,
        )
    }
                ...
                if let Some(closing_char) = chars.get(closing_index) {
                    if *closing_char == '\'' {
                        for _ in 0..=closing_index.saturating_sub(*index) {
                            self.highlighting.push(highlightling::Type::Character);
                            *index += 1;
                        }
                        return true;
                    }
            ...
            if let Some(next_char) = chars.get(index.saturating_add(1)) {
                if *next_char == '/' {
                    for _ in *index..chars.len() {
                        self.highlighting.push(highlightling::Type::Comment);
                        *index += 1;
                    }
                    return true;
                }
            ...
            if let Some(next_char) = chars.get(index.saturating_add(1)) {
                if *next_char == '*' {
                    ...
                    for _ in *index..closing_index {
                        self.highlighting
                            .push(highlightling::Type::MultilineComment);
                        *index += 1;
                    }
                    return true;
                }
    ...
    fn highlight_string(
        ...
        if opts.strings() && c == '"' {
            loop {
                self.highlighting.push(highlightling::Type::String);
                *index += 1;
    ...
    fn highlight_number(
        ...
        if opts.numbers() && c.is_ascii_digit() {
            ...
            loop {
                self.highlighting.push(highlightling::Type::Number);
                *index += 1;
                if let Some(next_char) = chars.get(*index) {
    ...
    pub fn highlight(
        ...
        if self.is_highlighted && word.is_none() {
            if let Some(hl_type) = self.highlighting.last() {
                if *hl_type == highlightling::Type::MultilineComment
                    && self.string.len() > 1
        ...
        while let Some(c) = chars.get(index) {
            ...
            self.highlighting.push(highlightling::Type::None);
            index += 1;
        }
